...
async function uploadImage(
  ...
  // Check cache first
  if (cache.has(imageRef.file)) {
    return cache.get(imageRef.file)!
  }

  const filePath = path.join(PDF_OUTPUT, imageRef.file)
  ...
  cache.set(imageRef.file, media.id as number)
  return media.id as number
}

/* ── Convert content blocks from def to Payload format ──── */
async function convertContentBlocks(
  ...
  for (const block of blocks) {
    switch (block.blockType) {
            ...
            ? {
                root: {
                  ...
                  children: [
                    {
                      type: 'paragraph',
                      children: [{ type: 'text', text: block.bodyText, version: 1 }],
                      version: 1,
                    },
                  ],
                  direction: 'ltr',
      ...
      case 'imageBlock': {
        const mediaId = await uploadImage(payload, block.image, imageCache)
        result.push({
          blockType: 'imageBlock',
          image: mediaId,
      ...
      case 'galleryBlock': {
        ...
        for (const img of block.images) {
          const mediaId = await uploadImage(payload, img, imageCache)
          galleryImages.push({
            image: mediaId,
            caption: img.alt,
          })
      ...
      case 'beforeAfterBlock': {
        const beforeId = await uploadImage(payload, block.beforeImage, imageCache)
        const afterId = await uploadImage(payload, block.afterImage, imageCache)
        result.push({
          blockType: 'beforeAfterBlock',
          heading: block.heading || undefined,
...
async function seed() {
  ...
  console.log(`   Deleted ${existingProjects.docs.length} existing projects`)

  const existingCategories = await payload.find({ collection: 'project-categories', limit: 100 })
  for (const cat of existingCategories.docs) {
    await payload.delete({ collection: 'project-categories', id: cat.id })
  }
  console.log(`   Deleted ${existingCategories.docs.length} existing categories`)
  ...
  console.log(`   Deleted ${existingMedia.docs.length} existing media`)
  ...
  for (const project of PROJECTS) {
    ...
    const heroMediaId = await uploadImage(payload, project.heroImage, imageCache)

    // Convert content blocks (uploads images as needed)
    console.log(`      Processing ${project.contentBlocks.length} content blocks...`)
    const contentBlocks = await convertContentBlocks(payload, project.contentBlocks, imageCache)
    ...
    await payload.create({
      ...
      data: {
        ...
        year: project.year,
        services: project.services.map(s => ({ service: s })),
        summary: project.summary,
        brief: project.brief,
        keyConsiderations: project.keyConsiderations?.map(c => ({ consideration: c })),
    ...
    console.log(`      ✅ Created "${project.title}" (${imageCache.size} total images uploaded)`)
  }
  ...
  console.log(`   🖼️  ${imageCache.size} images uploaded`)
